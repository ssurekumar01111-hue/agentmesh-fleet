...
Tests two contracts against the live deployed Cloud Run service:
  1. ctr-2026-005 — planted policy-violating contract -> 202 -> poll -> waiting_approval
  2. ctr-2026-001 — normal compliant contract         -> 202 -> poll -> completed
...
def trigger_review(base_url: str, contract_id: str) -> requests.Response:
...
def main():
    ...
    print("=" * 70)
    print("AgentMesh Legal Contract & NDA Reviewer — Async 202 Integration Test (Phase 15c)")
    print(f"Service URL: {base_url}")
    print("=" * 70)

    # ------------------------------------------------------------------
    # Test 1: Policy-violating contract (ctr-2026-005)
    # Expected: 202 -> poll -> waiting_approval, riskScore >= 0.60
    # ------------------------------------------------------------------
    print(f"\n--- TEST 1: Policy-violating contract ({VIOLATING_CONTRACT_ID}) ---")
    workflow_id = f"ctr-wf-{VIOLATING_CONTRACT_ID}"

    res = trigger_review(base_url, VIOLATING_CONTRACT_ID)
    ...
    if p1 and p2:
        ...
        if ctx.get("findings"):
            print(f"\n  Findings for {VIOLATING_CONTRACT_ID}:")
    ...
    # ------------------------------------------------------------------
    print(f"\n\n--- TEST 2: Normal compliant contract ({NORMAL_CONTRACT_ID}) ---")
    workflow_id_n = f"ctr-wf-{NORMAL_CONTRACT_ID}"

    res_n = trigger_review(base_url, NORMAL_CONTRACT_ID)

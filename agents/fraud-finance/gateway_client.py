...
SERVICE_ACCOUNT_EMAIL = f"agentmesh-fraud-finance@{PROJECT_ID}.iam.gserviceaccount.com"
...
class GatewayClient:
    ...
    def get_invoice(self, invoice_id: str) -> Optional[Dict[str, Any]]:
        data = self.call_gateway(
            target_resource="firestore:sandbox_invoices",
            collection_name="sandbox_invoices",
            action="read",
            payload={"docId": invoice_id}
        )
        return data

    def get_vendor(self, vendor_id: str) -> Optional[Dict[str, Any]]:
        data = self.call_gateway(
            target_resource="firestore:sandbox_vendors",
            collection_name="sandbox_vendors",
            action="read",
            payload={"docId": vendor_id}
        )
        return data

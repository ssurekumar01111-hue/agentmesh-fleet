...
SERVICE_ACCOUNT_EMAIL = f"agentmesh-expense-approval@{PROJECT_ID}.iam.gserviceaccount.com"


class GatewayClient:
    """Client for making all data/memory/workflow calls strictly through AgentMesh Gateway.

    Mirrors the pattern established in agents/fraud-finance/gateway_client.py.
    ...
    def get_expense(self, expense_id: str) -> Optional[Dict[str, Any]]:
        """Fetch a single expense report via Gateway (sandbox_expenses collection)."""

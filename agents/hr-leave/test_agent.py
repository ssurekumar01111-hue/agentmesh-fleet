...
Tests two leave requests against the live deployed Cloud Run service:
  1. lvr-2026-006 — planted policy-violating leave -> 202 -> poll -> waiting_approval
  2. lvr-2026-001 — normal compliant leave         -> 202 -> poll -> completed
...
def trigger_review(base_url: str, request_id: str) -> requests.Response:
...
def main():
    ...
    print("=" * 70)
    print("AgentMesh HR Leave Assistant Agent — Async 202 Integration Test (Phase 15c)")
    print(f"Service URL: {base_url}")
    print("=" * 70)

    # ------------------------------------------------------------------
    # Test 1: Policy-violating leave request (lvr-2026-006)
    # Expected: 202 -> poll -> waiting_approval, riskScore >= 0.60
    # ------------------------------------------------------------------
    print(f"\n--- TEST 1: Policy-violating leave request ({VIOLATING_REQUEST_ID}) ---")
    workflow_id = f"hr-wf-{VIOLATING_REQUEST_ID}"

    res = trigger_review(base_url, VIOLATING_REQUEST_ID)
    ...
    if p1 and p2:
        ...
        if ctx.get("findings"):
            print(f"\n  Findings for {VIOLATING_REQUEST_ID}:")
    ...
    # ------------------------------------------------------------------
    print(f"\n\n--- TEST 2: Normal compliant leave request ({NORMAL_REQUEST_ID}) ---")
    workflow_id_n = f"hr-wf-{NORMAL_REQUEST_ID}"

    res_n = trigger_review(base_url, NORMAL_REQUEST_ID)

...
sys.path.append(os.path.join(os.path.dirname(__file__), "agents", "compliance"))
from gateway_client import GatewayClient

AGENT_URL = "https://agentmesh-compliance-138003672216.asia-south1.run.app"
GATEWAY_URL = "https://agentmesh-gateway-138003672216.asia-south1.run.app"
PROJECT_ID = "agentmesh-fleet-2026"
SA_EMAIL = f"agentmesh-compliance@{PROJECT_ID}.iam.gserviceaccount.com"
...
def run_compliance_tests():
    print("=" * 80, flush=True)
    print("COMPLIANCE AGENT (PHASE 15b) END-TO-END PROOF VERIFICATION", flush=True)
    print("=" * 80, flush=True)
    ...
    init_payload = {
        ...
        "payload": {
            ...
            "data": {
                ...
                "involvedServiceAccounts": [SA_EMAIL, DASHBOARD_SA, f"agentmesh-fraud-finance@{PROJECT_ID}.iam.gserviceaccount.com"],
                "currentStep": "queued",
                "context": {"invoiceId": "inv-2026-007", "riskScore": 0.95},
    ...
    # ----------------------------------------------------
    # TEST 1: POST /review for workflowId
    # ----------------------------------------------------
    print(f"\n[STEP 1] Submitting async compliance review request for workflow '{workflow_id}'...", flush=True)
    headers = get_auth_headers()
    start_t = time.time()
    res = requests.post(f"{AGENT_URL}/review", json={"workflowId": workflow_id}, headers=headers, timeout=30)
    ...
    # ----------------------------------------------------
    print(f"\n[STEP 2] Polling Firestore for state transitions (queued -> running -> under_compliance_review/completed)...", flush=True)
    seen_states = {}
    ...
    while time.time() - poll_start < max_wait:
        ...
        if current_status in ["under_compliance_review", "completed", "running"]:
            # Keep polling until final decision or max time
            if current_status in ["under_compliance_review", "completed"]:
                final_wf = wf
                break
    ...
    assert "running" in seen_states or "under_compliance_review" in seen_states or "completed" in seen_states, "Workflow did not start running"
    print(f"[+] STEP 2 SUCCESS: Verified real state transitions with real timestamps!", flush=True)

    # ----------------------------------------------------
    # TEST 3: Idempotency Guard Test (Simulate duplicate Pub/Sub delivery)
    # ----------------------------------------------------
    print(f"\n[STEP 3] Testing Idempotency Guard (sending duplicate worker payload)...", flush=True)
    ...
    print(f"[+] STEP 3 SUCCESS: Idempotency guard prevented double-processing!", flush=True)

    # ----------------------------------------------------
    # TEST 4: Zero-Trust Denial Verification (POST /test-denied)
    # ----------------------------------------------------
    print(f"\n[STEP 4] Testing Zero-Trust HR Data Access Denial (/test-denied)...", flush=True)
    denied_res = requests.post(f"{AGENT_URL}/test-denied", json={}, headers=headers, timeout=30)
    print(f"    /test-denied Response Status: {denied_res.status_code}", flush=True)
    print(f"    /test-denied Response Body: {denied_res.text}", flush=True)

    assert denied_res.status_code == 200, f"Expected 200 OK from /test-denied, got {denied_res.status_code}"
    denied_json = denied_res.json()
    assert denied_json.get("status") == "completed", f"Expected status='completed', got {denied_json.get('status')}"
    gw_resp = denied_json.get("gatewayResponse", {})
    assert gw_resp.get("status_code") in (403, 401) or gw_resp.get("success") is False, "Expected Gateway to deny compliance agent read on sandbox_employees"
    print(f"[+] STEP 4 SUCCESS: Zero-Trust access policy enforcement verified!")

    print("\n" + "=" * 80, flush=True)
    print("COMPLIANCE AGENT PROOF PASSED PERFECTLY!", flush=True)
    print("=" * 80, flush=True)

...
sys.path.append(os.path.join(os.path.dirname(__file__), "agents", "expense-approval"))
from gateway_client import GatewayClient

AGENT_URL = "https://agentmesh-expense-approval-138003672216.asia-south1.run.app"
...
def run_expense_approval_tests():
    print("=" * 80, flush=True)
    print("EXPENSE APPROVAL AGENT (PHASE 15b) END-TO-END PROOF VERIFICATION", flush=True)
    print("=" * 80, flush=True)
    ...
    # ----------------------------------------------------
    # TEST 1: POST /review for expenseId
    # ----------------------------------------------------
    print(f"\n[STEP 1] Submitting async expense review request for expense '{expense_id}'...", flush=True)
    headers = get_auth_headers()
    start_t = time.time()
    res = requests.post(f"{AGENT_URL}/review", json={"expenseId": expense_id}, headers=headers, timeout=30)
    ...
    print(f"[+] STEP 3 SUCCESS: Final expense review result verified!", flush=True)
    ...
    print("\n" + "=" * 80, flush=True)
    print("EXPENSE APPROVAL AGENT PROOF PASSED PERFECTLY!", flush=True)
    print("=" * 80, flush=True)

if __name__ == "__main__":
    run_expense_approval_tests()

...
sys.path.append(os.path.join(os.path.dirname(__file__), "agents", "hr-leave"))
from gateway_client import GatewayClient

AGENT_URL = "https://agentmesh-hr-leave-138003672216.asia-south1.run.app"
GATEWAY_URL = "https://agentmesh-gateway-138003672216.asia-south1.run.app"
PROJECT_ID = "agentmesh-fleet-2026"
SA_EMAIL = f"agentmesh-hr-leave@{PROJECT_ID}.iam.gserviceaccount.com"
...
def run_hr_leave_tests():
    print("=" * 80, flush=True)
    print("HR LEAVE AGENT (PHASE 15b) END-TO-END PROOF VERIFICATION", flush=True)
    print("=" * 80, flush=True)

    request_id = "lvr-2026-001"
    ...
    # ----------------------------------------------------
    # TEST 1: POST /review for requestId
    # ----------------------------------------------------
    print(f"\n[STEP 1] Submitting async leave review request for request '{request_id}'...", flush=True)
    headers = get_auth_headers()
    start_t = time.time()
    res = requests.post(f"{AGENT_URL}/review", json={"requestId": request_id}, headers=headers, timeout=30)
    ...
    print(f"[+] STEP 3 SUCCESS: Final leave review result verified!", flush=True)
    ...
    print("\n" + "=" * 80, flush=True)
    print("HR LEAVE AGENT PROOF PASSED PERFECTLY!", flush=True)
    print("=" * 80, flush=True)
